	...
	while True:
		...
		while True: # Iterate through all flows in the packet
			...
			try:
				...
				logging.debug("Found flow ID " + str(flow_set_id) + ", length " + str(flow_set_length) + " at " + str(pointer))
			except:
			...
			logging.debug("Finshed, position " + str(pointer))
			
			if flow_set_id == 0: # Template flowset
				logging.debug("Unpacking template flowset " + str(flow_set_id) + ", position " + str(pointer))
				...
				logging.debug("Finished, position " + str(pointer))
			...
			# Flow data set
				...
				logging.debug("Unpacking data set " + str(flow_set_id) + ", position " + str(pointer))
				...
				if template_list[hashed_id]["Type"] == "Flow Data":
					...
					while data_position+4 <= (flow_set_length + (pointer-4)):
						...
						logging.debug("Data flow number " + str(flow_counter) + ", set ID " + str(flow_set_id) + " from " + str(sensor_address[0])) 
						...
						flow_dic.append(flow_index)
                                                logging.info(str(record_num) + ":" + str(flow_index))	
				...
				# Advance to the end of the flow
				pointer = (flow_set_length + pointer)-4
				logging.debug("Finished set " + str(flow_set_id) + ", position " + str(pointer))
